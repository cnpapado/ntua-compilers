...
%%%      valid Edsger programs.
%%%
%%% Generate the AST of a random program by:
%%%    proper_gen:pick(edsger_syntax_gen:program()).
%%% and repeat issuing this call as many times as you like.
%%%
%%% Alternatively, pretty-print a random Edsger program on stdout by:
%%%    edsger_pp:p().
%%% and repeat this until you are satisfied (or bored).
%%% 
%%% -------------------------------------------------------------------
-module(edsger_syntax_gen).
...
char_const() ->  % These are special-cased in edsger_pp!
  {'char', union(['a', '7', '\n', '\''])}.
